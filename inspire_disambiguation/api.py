...
def train_and_save_distance_model(
    ...
    Args:
        ...
        save_distance_model_path (str): Full path where trained model should be saved.
        sampled_pairs_size (int): How many pairs should be generated for the training.
            It should be multiple of 12.
    """
...
def cluster(ethnicity_model_path, distance_model_path, n_jobs, signature_block=None):
    ...
    Args:
        ...
        signature_block (str): Signature block for which cluster will be calculated.
            If set to None, it will run for all signatures.
    """
    LOGGER.info(
...
def cluster_from_redis(ethnicity_model_path, distance_model_path, n_jobs):
    ...
    while True:
        signature_block_data = redis.bzpopmin(
            conf['REDIS_PHONETIC_BLOCK_KEY'], conf.get("REDIS_TIMEOUT", 60)
        )
