...
class LiteratureSearch(Search):
    ...
    connection = connections.create_connection(
        hosts=[conf["ES_HOSTNAME"]], timeout=conf.get("ES_TIMEOUT", 60)
    )
...
def get_lit_records_query(signature_block, only_curated):
    ...
    Args:
        ...
        only_curated (bool): if `True` limits results of query to curated
    ...
    query = (
        ...
        .params(size=conf.get("ES_MAX_QUERY_SIZE", 9999), _source=SIGNATURE_FIELDS)
    )
    return query
...
def get_signatures(signature_block=None, only_curated=False):
    ...
    Raises:
        list: dicts of signatures which are matching `signature_block`.
...
def get_input_clusters(signatures, only_curated=True):
    ...
    for signature in signatures:
        # Ask Micha do we need here only curated ones?
