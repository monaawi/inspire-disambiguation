...
def sample_signature_pairs(curated_signatures, clusters, pairs_size=None):
    ...
    #
    if not pairs_size:
        pairs_size = conf["SAMPLED_PAIRS_SIZE"]
    blocks_and_uuids = []
    ...
    for signature in curated_signatures:
        ...
        blocks_and_uuids.append(
            (signature["signature_block"], signature["signature_uuid"])
        )
        author_names_by_signature_uuid[signature["signature_uuid"]] = signature[
            "author_name"
        ]
    ...
    iterations = 0
    pb = None
    if conf["DISPLAY_PROGRESS"]:
        pb = progressbar(
            length=pairs_size, label=f"Building {pairs_size} pairs of signatures"
        )
    while (
        any(count < pairs_size // 4 for count in counts.values())
    ...
    if iterations == max_iterations:
        raise IncompleteSamplingError(
            ...
            " Generated samples are probably unbalanced.".format(
                pairs_size, sum(counts.values())
            )
        )
