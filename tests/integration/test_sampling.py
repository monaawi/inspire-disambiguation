...
from inspire_disambiguation.core.ml.sampling import sample_signature_pairs, IncompleteSamplingError
...
def test_sample_signature_pairs(scan_mock, random_choice, es_record_with_many_curated_authors):
...
@patch("inspire_disambiguation.core.ml.sampling.random.choice")
@patch("inspire_disambiguation.core.es.readers.LiteratureSearch.scan")
def test_sample_signature_pairs_could_not_generate_pairs(scan_mock, random_choice, es_record_with_many_curated_authors):
    scan_mock.side_effect = [[es_record_with_many_curated_authors]]
    signatures = get_signatures()
    clusters = get_input_clusters(signatures)
    # it won't be able to generate the needed pairs because it can only find the same cluster, different name
    choices = [
                  ("JOhn", "94fc2b0a-dc17-42c2-bae3-ca0024079e52"),
                  "94fc2b0a-dc17-42c2-bae3-ca0024079e53",
              ] * 64
